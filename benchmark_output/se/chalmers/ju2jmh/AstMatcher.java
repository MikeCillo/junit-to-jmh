...
public class AstMatcher extends BaseMatcher<Node> {
    ...
    private static final PrinterConfiguration AST_PRINTER_CONFIG = new DefaultPrinterConfiguration();
    ...
    public void describeMismatch(Object item, Description description) {
        ...
        List<String> actualLines = actualAst.toString(AST_PRINTER_CONFIG).lines().collect(Collectors.toUnmodifiableList());
        ...
        for (int i = 0; i < diffRows.size(); i++) {
            ...
            switch(diffRow.getTag()) {
                case EQUAL:
                    description.appendText(NEWLINE).appendText("  ").appendText(diffRow.getOldLine());
                    break;
                case DELETE:
                    description.appendText(NEWLINE).appendText("- ").appendText(diffRow.getOldLine());
                    break;
                case INSERT:
                    description.appendText(NEWLINE).appendText("+ ").appendText(diffRow.getNewLine());
                    break;
                case CHANGE:
                    ...
                    for (int j = 0; j < oldCount; j++) {
                        description.appendText(NEWLINE).appendText("- ").appendText(changed.get(j).getOldLine());
                    }
                    for (int j = 0; j < newCount; j++) {
                        description.appendText(NEWLINE).appendText("+ ").appendText(changed.get(j).getNewLine());
                    }
                    break;
            }
        }
